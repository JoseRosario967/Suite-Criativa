...
export const UpscalerStudio: React.FC<UpscalerStudioProps> = ({ onAddToHistory, onClose, isWatermarkEnabled, activeWatermark, personalApiKey }) => {
  ...
  const handleUpscale = async () => {
    if (!sourceImage) {
        setError("Por favor, carregue uma imagem para ampliar a qualidade.");
        return;
    }
    setIsLoading(true);
    setError(null);
    ...
    try {
        const result = await upscaleImage(sourceImage, personalApiKey);
        ...
        setError(e.message || "Ocorreu um erro desconhecido ao ampliar a imagem.");
    } finally {
        setIsLoading(false);
    }
  };
  ...
  const handleDownload = (imageUrl: string) => {
    ...
    link.href = imageUrl;
    link.download = `imagem-ampliada-${Date.now()}.png`;
  ...
  const handleAddToHistoryClick = () => {
    if (resultImage) {
        onAddToHistory("Imagem com qualidade ampliada", resultImage);
    }
  };
  ...
  return (
    <div className="relative w-full">
      <button 
        ...
        aria-label="Fechar Estúdio de Super Resolução"
      >
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-1/3 flex flex-col space-y-6">
            <h2 className="text-3xl font-bold text-white">Estúdio de Super Resolução</h2>
            <p className="text-gray-400">
                Transforme imagens de baixa qualidade em obras de arte de alta definição. Carregue uma imagem e deixe a IA aumentar os detalhes.
            </p>
            {error && <ErrorAlert message={error} onDismiss={() => setError(null)} />}

            <div className="space-y-3">
                <label htmlFor="upscale-upload" className="block text-lg font-semibold text-gray-300">1. Carregar Imagem</label>
                <div 
                ...
                >
                    ...
                    <input id="upscale-upload" type="file" accept="image/*" onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                </div>
            </div>
            
            <button onClick={handleUpscale} disabled={isLoading || !sourceImage} className="w-full flex items-center justify-center px-6 py-3 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 disabled:bg-gray-600 disabled:cursor-not-allowed">
                {isLoading ? <Spinner /> : 'Ampliar Qualidade'}
            </button>
        </div>
        ...
        <div className="w-full lg:w-2/3 h-[75vh] bg-gray-800/50 border-2 border-gray-700 rounded-lg flex flex-col items-center justify-center overflow-hidden p-4">
            {isLoading ? (
                <div className="text-center"><Spinner size="lg" /><p className="mt-4 text-gray-400">A ampliar a imagem...</p></div>
                    ...
                    <div className="flex-grow grid grid-cols-2 gap-4">
                        <div className="flex flex-col items-center">
                         ...
                         <div className="flex flex-col items-center">
                            ...
                            <img src={resultImage} alt="Imagem ampliada" className="object-contain w-full h-full max-h-[60vh] rounded-md" />
                        </div>
                    </div>
                ...
                <div className="text-center text-gray-500">
                    <p className="font-semibold text-lg">O resultado ampliado aparecerá aqui</p>
